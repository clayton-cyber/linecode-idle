      ...
      <div className="px-3 sm:px-4 py-2 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                >
                  ...
                  <div className="space-y-0.5 max-h-48 overflow-y-auto">
                    {allLessons.filter(l => !l.isCustom).map((lesson) => (
                      <button
                        ...
                        className={`w-full px-2.5 py-1.5 rounded-md flex items-center justify-between text-left text-xs font-mono transition ${
                    ...
                    <>
                      ...
                      <div className="space-y-0.5 max-h-36 overflow-y-auto">
                        {customLessons.map((lesson) => (
                          <div
                            key={lesson.id}
                            className={`w-full px-2.5 py-1.5 rounded-md flex items-center justify-between text-left text-xs font-mono transition group ${
                              currentLesson.id === lesson.id
                                ? 'bg-blue-600 text-white font-bold'
                                : isDark ? 'hover:bg-[#1e1e1e]' : 'hover:bg-[#f0f3f6]'
                            }`}
                  ...
                  <div className="pt-2 mt-1 border-t space-y-1" style={{ borderColor: isDark ? '#3c3c3c' : '#e1e4e8' }}>
                    ...
                    <button
                      ...
                      className={`w-full py-1.5 px-2.5 rounded border text-xs font-medium flex items-center justify-center gap-1.5 transition ${
                        isDark ? 'border-[#3c3c3c] hover:bg-[#1e1e1e]' : 'border-[#d0d7de] hover:bg-[#f6f8fa]'
                      }`}
                    >
          ...
          <button
            ...
            className={`p-1.5 sm:px-2 sm:py-1.5 rounded-lg border transition text-xs flex items-center gap-1 ${
              isDark 
                ? 'bg-[#1e1e1e] hover:bg-[#2d2d2d] border-[#3c3c3c] text-[#cccccc]' 
                : 'bg-[#f6f8fa] hover:bg-[#eaeef2] border-[#d0d7de] text-[#24292f]'
            }`}
